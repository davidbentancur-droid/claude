...
1. **Ele tem senha, e falha fechado.** `middleware.ts` confere um cookie assinado com HMAC, chaveado pela `PAINEL_SENHA`. Sem a variável, ou com menos de 8 caracteres, a rota devolve 503 e não mostra nada. A página lista WhatsApp, profissão, orçamento e as respostas inteiras de gente que preencheu um formulário; isso não fica aberto por descuido de configuração. O HMAC sai do Web Crypto e não do `node:crypto` porque o middleware roda no Edge, onde o segundo não existe, e assim `lib/painel/sessao.ts` serve os dois lados sem duplicar nada. Era Basic Auth até 20/09: funcionava e era feio, a caixa do navegador não aceita identidade visual e não tinha como sair sem fechar o navegador.
...
3. **O estilo não segue a régua da casa, de propósito.** Gradiente e trapézio são proibidos no quiz e usados aqui, porque a régua existe pra peça que o cliente vê e esta é interna, desenhada em cima da referência de dashboard que o David mandou. A paleta é a da casa um tom abaixo, marrom quase preto com ouro, e a serifa fica só em título e número grande: rótulo, tabela, botão e resposta de cliente saem na sans, que é o que se lê rápido numa tela de trabalho.
...
Três coisas do lado da Anthropic que não são óbvias e já custaram 400:

1. **No Fable e no Mythos o raciocínio é sempre ligado.** Qualquer `thinking` explícito que não seja `adaptive` volta 400, **inclusive `disabled`**. `ProvedorAnthropic` omite o parâmetro nesses modelos em vez de mandar `disabled`, então `ENGINE_THINKING=disabled` vira sem efeito ali em vez de virar erro. `budget_tokens` e `temperature` também são 400 na família 5.
2. **Recusa não é erro.** Um classificador de segurança devolve 200 com conteúdo vazio e `stop_reason: "refusal"`. Este quiz recebe gente escrevendo sobre morte, doença e violência sofrida, que é a matéria-prima do método e também o assunto que faz classificador hesitar. Por isso a chamada leva `fallbacks` pro `claude-opus-5`, e o laço de tentativa corta na hora quando a cadeia inteira recusa: sem esse corte o retry mandaria as mesmas respostas e repetiria a recusa até acabar a cota. O pré-filtro de `lib/engine/risk.ts` continua sendo a primeira porta e não mudou.
3. **Preço.** `pnpm custo` mede o lead inteiro no provedor configurado e imprime o valor. Medido em 21/09 no gpt-5: US$ 0,354 por lead, com a saída respondendo por 93%. O Fable 5.1 é 8x a entrada e 5x a saída, então a conta muda de ordem de grandeza e vale medir antes de abrir tráfego.
...
7. **O kit de arte chegou pela metade, e o card tem dois estados.** Em 21/09 entraram nove dos vinte Movimentos (1, 4, 8, 9, 15, 16, 17, 18, 19), em 1264x848. A `frase_card` de cada um está copiada **da própria arte** pra dentro de `lib/movimentos.ts`, e é de lá que o componente desenha: o modelo continua proibido de escrever a frase, o que garante que o texto na tela e o texto dentro da imagem sejam o mesmo. Sem arte, `components/dossie/CardMovimento.tsx` monta o card só de texto na especificação que o David mandou (fundo #182B22, filete simples #A8836B, sem moldura dupla e sem capitular). A regra anterior, usar a arte do Ato como substituta, morreu: três imagens cobrindo vinte Movimentos diziam menos que um card honesto. Pra ligar uma arte nova: arquivo em `public/cards/` com o nome de `arquivoDoCard`, frase em `frase_card`, `tem_card: true`.
